...
Моя задача была "докерезировать" его. Что и было сделано. В дирикториях конкретных сервисов лежат Docker-файлы,а в корневой директории docker-compose -файл.
Так же подразумевается наличие .ENV-файла с секретными переменными, который отсутствует в данном перечне файлов (по понятным причинам).
